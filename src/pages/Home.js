...
const Home = () => {
	...
	const { headerImages, domesticTours, internationalTours, trendingTours } = useSelector((state) => state.homepageData);
	...
	useEffect(() => {
		async function fetchHomepageData() {
			try {
				...
				const res = await fetch(process.env.REACT_APP_REALTIME_DB_LINK);
				...
				dispatch(homepageSliceAction.setTours({ headerImages, domesticTours, internationalTours, trendingTours }));
				setIsLoading(false);
			} catch (error) {
				setIsLoading(false);
		...
		if (
			...
			fetchHomepageData();
	}, [headerImages, domesticTours, internationalTours, trendingTours]);
